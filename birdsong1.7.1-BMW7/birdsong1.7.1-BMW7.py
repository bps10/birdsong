...
############## BEGIN CONFIG ##########################################
version         = '2012-04-26'  # version of this module
CONTACT_EMAIL   = 'bps10@uw.edu'
TESTING = 1     # myexit
############## END CONFIG ############################################
...
try:
    ...
    print 'Please email %s for assistance.' % CONTACT_EMAIL
...
try:
    ...
    print 'Software configuration error. You need to install the wx package.'
    print 'Please email %s for assistance.' % CONTACT_EMAIL
    raw_input('Press enter to close this window.')
    sys.exit(1)
...
#DEFAULT_DIR = 'C:\\Users\\Brandon\\Desktop\\BirdSongAnalysis'
DEFAULT_DIR = 'BirdSongAnalysis'

# BIRD INFO #
#BIRDNAME = 'Bird 1958'
#CONDITION = 1 ## 0 = Baseline, 1 = Post Manipulation
...
# ANALYSIS OPTIONS #
#MAXFILE = 3 ## Maximum number of songs to be analyzed
...
def MsgDlg(parent, title, str):
    dlg = wx.MessageDialog(parent, str, title, wx.OK | wx.ICON_INFORMATION)
    dlg.ShowModal()
    dlg.Destroy()
...
class ControlPanel(wx.Panel):
    ...
    def __init__(self, parent, frame):
        ...
        # BirdSongAnalysis folder
        gbs.Add( wx.StaticText(self, -1, "BirdSongAnalysis folder"),
                 (row,0), (1,1), wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        self.birdsong_folder = wx.TextCtrl(self, -1, DEFAULT_DIR, size=(250,-1))
        gbs.Add( self.birdsong_folder,
                 (row,1), (1,1), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        b = wx.Button(self, -1, "Select")
        self.Bind(wx.EVT_BUTTON, self.OnSelectDir, b)
        gbs.Add( b, (row,2), (1,1), flag=wx.ALIGN_LEFT|wx.ALIGN_CENTER_VERTICAL)
        row += 1

        # Condition
        self.condition = wx.RadioBox(self, -1, "Condition", size=wx.DefaultSize,  
            choices=['Baseline','Post Manipulation'], majorDimension=2, style=wx.RA_SPECIFY_ROWS)
        gbs.Add( self.condition, (row,col), (2,2), wx.ALIGN_LEFT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 2

        # Bird folder
        gbs.Add( wx.StaticText(self, -1, "Bird folder"),
                 (row,0), (1,1), wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        self.bird_folder = wx.TextCtrl(self, -1, "Bird 1958", size=(100,-1))
        gbs.Add( self.bird_folder,
                 (row,1), (1,1), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        # DAY = 'Feb 2' ## Not important if Condition is 0
        gbs.Add( wx.StaticText(self, -1, "Day"),
                 (row,0), (1,1), wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        self.day = wx.TextCtrl(self, -1, "Feb 2", size=(100,-1))
        gbs.Add( self.day,
                 (row,1), (1,1), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        #SYLLABLES = 5 ## Number of syllables to be analyzed
        gbs.Add( wx.StaticText(self, -1, "Num syllables"),
                 (row,0), (1,1), wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        self.num_syllables = wx.TextCtrl(self, -1, "5", size=(nSizeNumBox,-1))
        gbs.Add( self.num_syllables,
                 (row,1), (1,1), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        #MAXFILE = 3 ## Maximum number of songs to be analyzed
        gbs.Add( wx.StaticText(self, -1, "Max num songs to analyze"),
                 (row,0), (1,1), wx.ALIGN_RIGHT|wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        self.max_num_songs = wx.TextCtrl(self, -1, "3", size=(nSizeNumBox,-1))
        gbs.Add( self.max_num_songs,
                 (row,1), (1,1), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        # Automatically compare
        self.analysis = wx.CheckBox(self, -1, 'Automatically compare Exp Condition to Baseline', size=(-1,-1))
        gbs.Add( self.analysis,
                 (row,0), (1,2), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        #DISPLAYSYLL = 0 ## Control whether syllables are displayed: 0=No, 1=Yes
        self.display_syll = wx.CheckBox(self, -1, 'Display syllables', size=(-1,-1))
        gbs.Add( self.display_syll,
                 (row,0), (1,2), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        #DISPLAYSONG = 0 ## Control whether songs are displayed: 0=No, 1=Yes
        self.display_song = wx.CheckBox(self, -1, 'Display song', size=(-1,-1))
        gbs.Add( self.display_song,
                 (row,0), (1,2), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1

        #SAVESYLL = 0 ## 0 = Do Not save syllable data into csv files, 1 = Yes
        self.save_syll = wx.CheckBox(self, -1, 'Save syllable data into csv files', size=(-1,-1))
        gbs.Add( self.save_syll,
                 (row,0), (1,2), wx.ALIGN_LEFT |wx.ALIGN_CENTER_VERTICAL|wx.ALL)
        row += 1
        ...
        self.Bind(wx.EVT_BUTTON, frame.OnRun, b)
    ...
    def OnSelectDir(self, evt):
        dlg = wx.DirDialog(self, "Choose a directory:",
                           self.birdsong_folder.GetValue(),
                          style=wx.DD_DEFAULT_STYLE
                           | wx.DD_DIR_MUST_EXIST
                           #| wx.DD_CHANGE_DIR
                           )
        if dlg.ShowModal() == wx.ID_OK:
            self.birdsong_folder.SetValue(dlg.GetPath())
...
class MyFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title):
        wx.Frame.__init__(self, parent, -1, title,
                         wx.DefaultPosition, wx.Size(600, 500)) # width, height
    ...
    def OnAbout(self, event):
        dlg = wx.MessageDialog(self, "This program ...\n"
                                    "Written January 2012 by Brian Schmidt (bps10@uw.edu)",
                              "About Birdsong", wx.OK | wx.ICON_INFORMATION)
        dlg.ShowModal()
        dlg.Destroy()
    ...
    def OnRun(self, evt):
        try:
            ...
            # Must execute blocking show(), so that execution pauses until user
            # deletes his plots, *otherwise the program will freeze when a plot is moved*.
            # Placed in try/finally block so that it gets called even if an exception occurs.
            # The following will not block if no plots have been created.
            MsgDlg(self, 'INFO', 'Please close this, then close your plots before doing anything else.')
            print 'before blocking show()'
            plt.show()
            print 'after blocking show()'

    def Run(self):
        global winFreq, THRESHOLD, DISPLAYSONG, t

        # get string vars from control panel
        BIRDNAME =  self.controlPanel.bird_folder.GetValue()
        CONDITION = self.controlPanel.condition.GetStringSelection()
        DAY = self.controlPanel.day.GetValue()

        # booleans (checkboxes)
        ANALYSIS = self.controlPanel.analysis.GetValue()
        DISPLAYSYLL = self.controlPanel.display_syll.GetValue()
        DISPLAYSONG = self.controlPanel.display_song.GetValue()
        SAVESYLL = self.controlPanel.save_syll.GetValue()

        # numbers must be converted from strings
        SYLLABLES = int(self.controlPanel.num_syllables.GetValue())
        MAXFILE = int(self.controlPanel.max_num_songs.GetValue())

        os.chdir(self.controlPanel.birdsong_folder.GetValue())
        ...
        elif CONDITION == 'Post Manipulation':
        ...
        try:
            dir_to_make = FILEDIR + 'Analysis'
            os.makedirs(dir_to_make)
        except:
            ErrorDlg(self, 'ERROR creating \n%s\n(does it already exist?)' % dir_to_make)
            return
        if SAVESYLL:
            try:
                dir_to_make = FILEDIR + 'SyllCSV'
                os.makedirs(dir_to_make)
            except:
                ErrorDlg(self, 'ERROR creating \n%s\n(does it already exist?)' % dir_to_make)
                return
        ...
        #    else:
        #        m = 5
        #    Syll_duration[i] = np.zeros((len(files)*m,1))
        #    Syll_entropy[i] = np.zeros((len(files)*4,200))
        #    Syll_freq[i] = np.zeros((len(files)*4,200))
        #    Syll[i] = [Syll_duration[i], Syll_entropy[i], Syll_freq[i]]
        #    Syll_full.append(Syll[i])
        ...
        ##READ FILES##
        if CONDITION == 'Post Manipulation' and ANALYSIS:
